#!/usr/bin/env python3
"""Fix OpenAI API compatibility in CAMEL model_backend.py"""

import subprocess

# The fixed code
fixed_code = '''# =========== Copyright 2023 @ CAMEL-AI.org. All Rights Reserved. ===========
...
with open('model_backend_fixed.py', 'w', encoding='utf-8', newline='\n') as f:
    f.write(fixed_code)

print("Fixed file created: model_backend_fixed.py")
print("Now copying to Docker container...")

# Copy to Docker container
result = subprocess.run([
    'docker', 'cp', 
    'model_backend_fixed.py',
    'codeagent-scanner-backend:/app/camel/model_backend.py'
], capture_output=True, text=True)

if result.returncode == 0:
    print("✓ Successfully copied fixed file to container")
else:
...
if result.returncode == 0:
    print("✓ File syntax validated successfully")
else:
    print(f"✗ Syntax error in fixed file: {result.stderr}")
    exit(1)

print("\n✓ OpenAI API fix applied successfully!")
print("Restart the backend container to apply changes.")
